...
public class Pract {
    ...
    public static int[] determinaPuntajePorPais(int[][] puntajes) {
        int[]puntajetotal=new int[puntajes.length];
        for (int i = 0; i < puntajes.length; i++) {
            for (int j = 0; j < puntajes[0].length; j++) {
                if(j<5){
                    puntajetotal[i]+=puntajes[i][j]*0.6/4;
                }else{
                    puntajetotal[i]+=puntajes[i][j]*0.4/3;
                }
            }
        }
        return puntajetotal;
    ...
    public static String determinaPaisConMedallaDeOro(int [] puntajeTotalPorPais,String [] paises) {
        ...
        for (int i = 0; i < puntajeTotalPorPais.length; i++) {
            if (puntajeTotalPorPais[i]>=mayor){
                mayor=puntajeTotalPorPais[i];
            }
        }
        return paises[mayor];
    }
    ...
    public static String [] determinaPaisesConRecord(int []puntajeTotalPorPais,String []paises,int puntajeRecord) {
        String [] paisesrecord=new String[paises.length];
        int cont=0;
        for (int i = 0; i < puntajeTotalPorPais.length; i++) {
            if (puntajeTotalPorPais[i]>=puntajeRecord){
                paisesrecord[cont]=paises[i];
                cont++;
            }
        }
        if(paisesrecord[0]==null) {
            return null;
        }else {
            return paisesrecord;
        }
